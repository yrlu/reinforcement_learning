  ...
  def learn(self, train_steps, sess):
    ...
    if len(self.mem) > self.mem_size/20:
      ...
      for i in xrange(train_steps):
        ...
        sample_idx = random.sample(range(1,len(self.mem)), self.batch_size)
        samples = []
        for idx in sample_idx:
          cur_state = np.stack([self.mem[idx][0], self.mem[idx-1][0]], axis=2)

          # import matplotlib.pyplot as plt
          # plt.ion()
          # plt.imshow(cur_state[:,:,0])
          # plt.show()
          # plt.pause(0.001)
          # raw_input()
          # plt.imshow(cur_state[:,:,1])
          # plt.show()
          # plt.pause(0.001)
          # raw_input()


          action = self.mem[idx][1]
          next_state = np.stack([self.mem[idx][2], self.mem[idx-1][2]], axis=2)
          reward = self.mem[idx][3]
          done = self.mem[idx][4]
          samples.append([cur_state, action, next_state, reward, done])
